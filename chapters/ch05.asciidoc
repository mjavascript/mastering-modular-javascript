...
> Another benefit worth pointing out: Object.assign() is a footgun, in that if you forget to pass an empty object literal as the first argument for this use case, you end up mutating the object. With object spread, there is no way to accidentally mutate anything.
...
> Side note: In Node.js v10 fs/promises is available.
...
function onEvent(target, eventType, listener) {
  if (!emitters.has(target)) {
    emitters.set(target, {})
    // Why an object? You’re using it like a Map. It should be a Map.
  }
...
> Note: it’s not quite a subset yet, but we’re getting there: https://github.com/tc39/proposal-json-superset :) (It’s fine to not mention this; by the time the book gets published the proposal might be at Stage 4.)
