...
Thinking in API-first terms can help, because we'd have a decent idea of the kind of API surface that we want, and armed with that we can decide how we want to allow consumers to interact with the component.
...
Open-source software components often get feature requests which are overly specific to the needs of that particular user. Taking feature requests or requirements at face value is not enough, instead we need to dive deeper and find commonalities between the feature that's being requested, features that we may have planned for our roadmap, and features we might want to adapt our component to support in the future.

It's important for a component to satisfy the needs of most of its consumers, but this doesn't mean we should attempt to satisfy use cases one by one, or in isolation.

.. At the same time, abstractions can be a costly way of merging use cases ..





- more prose around the above
- stories with rome/dragula-like cases
