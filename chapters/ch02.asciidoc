...
> Same comment as before: use extensions where applicable
...
> Okay, I’ll stop making this comment now. Extensions everywhere please :)
...
> The return value is not used in this example. Maybe assign it to const api or to window.api? By itself the `return` is not useful otherwise.

When exposing touchpoints on an interface, it's important to gauge if consumers need the touchpoint at all, how it helps them, and whether it could be made simpler. For instance, it might be the case that instead of exposing several different touchpoints the user can pick from, they're better off with a single touchpoint that leverages the appropriate code path based on provided inputs while, at the same time, the component would couple a smaller part of its implementation to its interface.
...
> Maybe point out that `== null` matches both `null` and `undefined` here. I think readers might not realize it unless you explicitly spell it out.
...
> JSON string → JSON-formatted string
> There is no such thing as a “JSON object” or “JSON string”
...
> Have you considered using async/await over vanilla promises by default? IMHO it makes examples cleaner and easier to follow. (Same for every other promise example in this book.)
