...
> This example is begging for the pipeline operator! :) (I don’t expect it to be mentioned though as it’s still in the early stages of standardization.)
...
function absolutizeHtml(html, origin) {
  ...
  $dom.find('a[href]').each(function () {
    ...
    // In the DOM, you could just use this.href, which always returns a normalized URL string:
    const absolute = this.href;
    // No need for any of this other code. Maybe use a different example?
  ...
  $dom.find('img[src]').each(function () {
    ...
    // In the DOM, you could just use this.src, which always returns a normalized URL string:
    const absolute = this.src;
    // No need for any of this other code. Maybe use a different example?
...
> If it’s an “object map” then it probably should be a Map. Maybe at least mention `Map` here?
